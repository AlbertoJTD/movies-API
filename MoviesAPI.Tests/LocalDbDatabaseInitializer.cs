	...
	public class LocalDbDatabaseInitializer
	{
		...
		public static ApplicationDbContext GetDbContextLocalDb(bool beginTransaction = true)
		{
			var options = new DbContextOptionsBuilder<ApplicationDbContext>()
				.UseSqlServer($"Server=MSI;Database={_dbName};Integrated Security=True;TrustServerCertificate=True",
		...
		{
			var fileNames = GetDbFiles(Master, $@"SELECT [physical_name] FROM [sys].[master_files]
												WHERE [database_id] = DB_ID('{_dbName}')");

			if (fileNames.Any())
			{
				ExecuteCommand(Master, $@"ALTER DATABASE [{_dbName}] SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
		...
		static string Filename => Path.Combine(Path.GetDirectoryName(
			typeof(LocalDbDatabaseInitializer).GetTypeInfo().Assembly.Location),$"{_dbName}.mdf"
		);
	}
}
